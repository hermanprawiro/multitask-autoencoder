...
class Autoencoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_class):
        ...
        self.decoder = nn.Linear(20, self.input_size)
        self.classifier = nn.Linear(20, self.num_class)
...
def train(loader, model, optimizer, criterion, writer, epoch):
    ...
    for i, (inputs, targets) in enumerate(loader):
        logits, reconstruction, mu, logvar = model(inputs)
        ...
        if i % 50 == 0:
            print('Epoch {0} [{1}/{2}]\t'
                ...
                'KLD Loss {kldloss.val:.4f} ({kldloss.avg:.4f})\t'
                'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                epoch + 1, i + 1, total_iter, 
                batch_time=batch_time, data_time=data_time, loss=losses,
                closs=classification_losses, rloss=reconstruction_losses, kldloss=kldivergence_losses,
                top1=top1, top5=top5)
            )
